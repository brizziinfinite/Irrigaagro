...
export default function PivoDetailError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[pivo-detail] Error boundary:', error)
  }, [error])
  ...
  return (
    <div style={{ padding: '60px 40px', textAlign: 'center', maxWidth: 480, margin: '0 auto' }}>
      ...
      <h2 style={{ color: '#e2e8f0', fontSize: 18, fontWeight: 700, marginBottom: 8 }}>
        Erro ao carregar pivô
      </h2>
      <p style={{ color: '#8899aa', fontSize: 13, marginBottom: 24, lineHeight: 1.6 }}>
        {error.message || 'Ocorreu um erro inesperado ao carregar o detalhe do pivô.'}
      </p>
      <button
        onClick={reset}
        style={{
