...
function TimelineChart({ records, threshold = 70 }: { records: DailyManagement[]; threshold?: number }) {
  ...
  const data = [...records]
    ...
    .map(r => ({
      ...
      irrigation: r.actual_depth_mm ?? 0,
      adc: r.field_capacity_percent ?? 0,
    }))

  const hasIrrigation = data.some(d => d.irrigation > 0)

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (!active || !payload?.length) return null
    return (
      <div style={{ background: 'rgba(10,18,28,0.97)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '12px 16px', boxShadow: '0 8px 32px rgba(0,0,0,0.5)', backdropFilter: 'blur(12px)', minWidth: 160 }}>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 700, color: '#e2e8f0', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: 6 }}>{label}</p>
        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
        {payload.map((entry: any, i: number) => {
          if (entry.value === 0 && (entry.dataKey === 'rainfall' || entry.dataKey === 'irrigation')) return null
          const isAdc = entry.dataKey === 'adc'
          const isIrrig = entry.dataKey === 'irrigation'
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
              <div style={{ width: 8, height: 8, borderRadius: 2, background: entry.color }} />
              <span style={{ fontSize: 11, color: '#8899aa', width: 52 }}>{entry.name}:</span>
        ...
        {/* Zona de estresse contextual */}
        {(() => {
          const adcEntry = payload.find((p: any) => p.dataKey === 'adc')
          if (!adcEntry) return null
          const pct = Number(adcEntry.value)
          const zone = pct >= threshold * 1.15 ? { label: 'Zona Segura', color: '#22c55e' }
  ...
  return (
    <div style={{ background: 'linear-gradient(160deg, #0a1218, #0f1923)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.3)' }}>
      {/* Header */}
      <div style={{ padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: '#e2e8f0' }}>Histórico — Últimos 30 dias</span>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: '#556677' }}><div style={{ width: 10, height: 3, background: '#f59e0b', borderRadius: 2 }} /> ETo</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: '#556677' }}><div style={{ width: 10, height: 3, background: '#06b6d4', borderRadius: 2 }} /> ETc</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: '#556677' }}><div style={{ width: 8, height: 8, background: 'rgba(255,255,255,0.6)', borderRadius: 2 }} /> Chuva</span>
          {hasIrrigation && <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: '#556677' }}><div style={{ width: 8, height: 8, background: 'rgba(34,211,238,0.7)', borderRadius: 2 }} /> Irrigação</span>}
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: '#556677' }}><div style={{ width: 10, height: 3, background: '#0093D0', borderRadius: 2 }} /> Umidade %</span>
        </div>
      </div>
          ...
          <ComposedChart data={data} margin={{ top: 8, right: 4, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="moistureArea" x1="0" y1="1" x2="0" y2="0">
                ...
                <stop offset={`${threshold}%`} stopColor="rgba(0,147,208,0.10)" />
                <stop offset="100%" stopColor="rgba(0,147,208,0.10)" />
              </linearGradient>
            ...
            {/* Umidade (área principal, eixo direito) */}
            <Area yAxisId="right" type="monotone" dataKey="adc" name="Umidade" fill="url(#moistureArea)" stroke="url(#moistureStroke)" strokeWidth={2.5} dot={{ r: 0 }} activeDot={{ r: 4, stroke: '#0f1923', strokeWidth: 2 }} />

            {/* Chuva */}
            <Bar yAxisId="left" dataKey="rainfall" name="Chuva" fill="rgba(200,220,255,0.65)" radius={[3, 3, 0, 0]} maxBarSize={16} />

            {/* Irrigação aplicada (destaque ciano) */}
            {hasIrrigation && (
              <Bar yAxisId="left" dataKey="irrigation" name="Irrigação" fill="rgba(34,211,238,0.75)" radius={[3, 3, 0, 0]} maxBarSize={16} />
            )}

            {/* ETc e ETo */}
            <Line yAxisId="left" type="monotone" dataKey="eto" name="ETo" stroke="#f59e0b" strokeWidth={1.5} dot={{ r: 0 }} activeDot={{ r: 3, stroke: '#0f1923', strokeWidth: 2 }} strokeDasharray="5 3" />
            <Line yAxisId="left" type="monotone" dataKey="etc" name="ETc" stroke="#06b6d4" strokeWidth={1.5} dot={{ r: 0 }} activeDot={{ r: 3, stroke: '#0f1923', strokeWidth: 2 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
