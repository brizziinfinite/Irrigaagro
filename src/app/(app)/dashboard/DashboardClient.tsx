...
const PivotMap = dynamic(
  ...
  { ssr: false, loading: () => (
    <div style={{
      height: 340, borderRadius: 16, background: '#111f14',
      border: '1px solid #1f3022', display: 'flex', alignItems: 'center',
      justifyContent: 'center', color: '#3a5240', fontSize: 13,
    }}>
      Carregando mapa…
    </div>
  )}
...
const STATUS_CONFIG: Record<IrrigationStatus, {
  ...
  sem_safra: { label: 'Sem safra',     color: '#535c3e', bg: 'rgb(83 92 62 / 0.12)',    border: 'rgb(83 92 62 / 0.25)',   icon: Info,          desc: 'Nenhuma safra ativa' },
}
    ...
    <div className="flex flex-col gap-6 max-w-2xl mx-auto py-8">
      <div style={{ textAlign: 'center', padding: '0 16px' }}>
        <div style={{
          ...
          background: 'linear-gradient(135deg, #166502, #4a9e1a)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 4px 20px rgb(74 158 26 / 0.4)',
        }}>
          <Droplets size={32} className="text-white" strokeWidth={2} />
        </div>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#ecefec', marginBottom: 10 }}>Bem-vindo ao IrrigaAgro</h1>
        <p style={{ fontSize: 15, color: '#7a9e82', lineHeight: 1.6 }}>
          Siga os passos abaixo para configurar seu sistema de manejo hídrico baseado no método FAO-56.
        </p>
      </div>
      <div style={{ background: '#111f14', border: '1px solid #1f3022', borderRadius: 20, padding: '8px 0', overflow: 'hidden' }}>
        {steps.map((step, i) => (
          <Link key={step.num} href={step.href} style={{
            display: 'flex', alignItems: 'center', gap: 16, padding: '18px 24px',
            textDecoration: 'none', borderBottom: i < steps.length - 1 ? '1px solid #1f3022' : 'none',
          }}
          ...
          >
            <div style={{
              ...
              background: 'rgb(74 158 26 / 0.12)', border: '1px solid rgb(74 158 26 / 0.25)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 14, fontWeight: 700, color: '#4a9e1a',
            }}>{step.num}</div>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 14, fontWeight: 600, color: '#ecefec' }}>{step.title}</p>
              <p style={{ fontSize: 12, color: '#535c3e', marginTop: 2 }}>{step.desc}</p>
            </div>
            <ArrowRight size={16} style={{ color: '#3a5240', flexShrink: 0 }} />
          </Link>
        ))}
      </div>
    </div>
...
function MiniTimeline({ days, time360h, threshold }: {
  ...
  return (
    <div style={{ borderTop: '1px solid #1a2e1d', paddingTop: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
        <Clock size={11} style={{ color: '#3a5240' }} />
        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: '#3a5240' }}>
          Projeção 7 dias
        </span>
      </div>
        ...
        {days.map((day, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
              {/* Label dia */}
              <span style={{ fontSize: 9, color: '#535c3e', textTransform: 'capitalize', whiteSpace: 'nowrap' }}>{label}</span>
              ...
              <div style={{
                ...
                <div style={{ padding: '5px 3px 6px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: needsIrr ? '#ecefec' : '#3a5240', fontFamily: 'var(--font-mono)' }}>
                      {depth > 0 ? depth.toFixed(1) : '0'}
                    </span>
                    <span style={{ fontSize: 8, color: '#3a5240' }}>mm</span>
                  </div>
                  {/* Velocidade */}
                  <div style={{ fontSize: 9, fontWeight: 800, fontFamily: 'var(--font-mono)', lineHeight: 1, color: needsIrr ? '#4a9e1a' : '#3a5240' }}>
                    {speed > 0 ? `${speed}%` : '0%'}
                  </div>
                  {/* Duração */}
                  <div style={{ fontSize: 8, fontFamily: 'var(--font-mono)', color: needsIrr ? '#7a9e82' : '#3a5240' }}>
...
function PivotCard({ pivot, hasActiveSeason, lastManagement, projection, diagnostic }: PivotCardProps) {
  ...
  return (
    <div style={{
      background: '#111f14',
      border: `1px solid ${needsIrrig && hasActiveSeason ? cfg.border : '#1f3022'}`,
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{
            ...
            <CircleDot size={15} style={{ color: '#4a9e1a' }} />
          </div>
          <div>
            <p style={{ fontSize: 14, fontWeight: 700, color: '#ecefec' }}>{pivot.name}</p>
          <p style={{ fontSize: 11, color: '#535c3e', marginTop: 1 }}>
              {pivot.farms?.name ?? 'Sem fazenda'}
            {m?.date && (
                <span style={{ marginLeft: 6, color: '#3a5240' }}>
      ...
      {diagnostic && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          <div style={{ background: '#162219', borderRadius: 8, padding: '8px 10px' }}>
            <p style={{ fontSize: 9, color: '#3a5240', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Clima</p>
            <p style={{ fontSize: 11, fontWeight: 600, color: '#becec0', marginTop: 4 }}>{diagnostic.climateRouteLabel}</p>
          </div>
          <div style={{ background: '#162219', borderRadius: 8, padding: '8px 10px' }}>
            <p style={{ fontSize: 9, color: '#3a5240', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Automação</p>
            <p style={{ fontSize: 11, fontWeight: 600, color: '#becec0', marginTop: 4 }}>{diagnostic.automationStatus}</p>
          </div>
        </div>
      )}
      ...
      {hasActiveSeason && (
        <div style={{
          borderRadius: 10,
          background: needsIrrig ? cfg.bg : '#162219',
          border: `1px solid ${needsIrrig ? cfg.border : '#1a2e1d'}`,
          overflow: 'hidden',
        }}>
          {/* Header */}
          <div style={{
            ...
            padding: '7px 12px',
            borderBottom: `1px solid ${needsIrrig ? cfg.border : '#1a2e1d'}`,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <Droplets size={11} style={{ color: needsIrrig ? cfg.color : '#3a5240' }} />
              <span style={{ fontSize: 10, fontWeight: 700, color: needsIrrig ? cfg.color : '#535c3e' }}>
                {needsIrrig ? 'Irrigação Recomendada' : 'Irrigação'}
              </span>
            </div>
            {recIsProjected && (
              <span style={{ fontSize: 9, color: '#3a5240', background: '#1a2e1d', padding: '2px 7px', borderRadius: 20 }}>
                projeção
              </span>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr' }}>
            {[
              { label: 'Lâmina',     value: recDepth    != null ? recDepth.toFixed(1)   : '—', unit: 'mm', color: needsIrrig ? '#ecefec' : '#535c3e' },
              { label: 'Velocidade', value: recSpeed    != null ? String(recSpeed)       : '—', unit: '%',  color: needsIrrig ? '#4a9e1a' : '#535c3e' },
              { label: 'Duração',    value: recDuration != null ? recDuration.toFixed(1) : '—', unit: 'h',  color: needsIrrig ? '#ecefec' : '#535c3e' },
            ].map(({ label, value, unit, color }, i) => (
              <div key={label} style={{
                padding: '10px 8px', textAlign: 'center',
                borderRight: i < 2 ? `1px solid ${needsIrrig ? cfg.border : '#1a2e1d'}` : 'none',
              }}>
                <p style={{ fontSize: 9, color: '#3a5240', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 3 }}>
                  {label}
                </p>
                <p style={{ fontSize: 20, fontWeight: 800, color, fontFamily: 'var(--font-mono)', lineHeight: 1 }}>
                  {value}
                </p>
                <p style={{ fontSize: 9, color: '#3a5240', marginTop: 2 }}>{unit}</p>
              </div>
            ))}
          </div>
          ...
          {/* Footer: OK */}
          {!needsIrrig && (
            <div style={{ padding: '5px 12px', borderTop: '1px solid #1a2e1d', display: 'flex', alignItems: 'center', gap: 6 }}>
              <CheckCircle2 size={10} style={{ color: '#22c55e' }} />
              <span style={{ fontSize: 10, color: '#535c3e' }}>Solo OK — sem necessidade de irrigação hoje</span>
            </div>
          )}
        </div>
      )}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
          <span style={{ fontSize: 11, color: '#535c3e' }}>Capacidade de Campo</span>
          <span style={{ fontSize: 12, fontWeight: 700, fontFamily: 'var(--font-mono)', color: pct !== null ? cfg.color : '#535c3e' }}>
            {pct !== null ? `${pct.toFixed(0)}%` : '—'}
          </span>
        </div>
        <div style={{ height: 7, background: '#162219', borderRadius: 99, overflow: 'hidden', position: 'relative' }}>
          {/* Marcador limiar */}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3 }}>
          <span style={{ fontSize: 9, color: '#3a5240' }}>0%</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 6 }}>
        {[
          ...
          { icon: CloudRain, label: 'Chuva', value: fmtVal(m?.rainfall_mm), unit: 'mm', color: '#7a9e82' },
          { icon: Zap,       label: 'Kc',    value: fmtVal(m?.kc, 2),       unit: '',   color: '#a78bfa' },
        ].map(({ icon: Icon, label, value, unit, color }) => (
          <div key={label} style={{ background: '#162219', borderRadius: 8, padding: '8px 6px', textAlign: 'center' }}>
            <Icon size={12} style={{ color: m ? color : '#3a5240', margin: '0 auto 3px' }} />
            <p style={{ fontSize: 15, fontWeight: 700, color: '#ecefec', lineHeight: 1, fontFamily: 'var(--font-mono)' }}>
              {value}
            </p>
            <p style={{ fontSize: 9, color: '#3a5240', marginTop: 2 }}>{label}{unit ? ` (${unit})` : ''}</p>
          </div>
        ))}
      </div>
      ...
      {!hasActiveSeason && (
        <Link href="/safras" style={{
          ...
          background: 'rgb(74 158 26 / 0.1)', border: '1px solid rgb(74 158 26 / 0.2)',
          color: '#4a9e1a', textDecoration: 'none',
        }}>
  ...
  return (
    <div className="flex flex-col gap-5 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold" style={{ color: '#ecefec' }}>Dashboard</h1>
      ...
      {Object.entries(grouped).map(([farmName, farmPivots]) => (
        <div key={farmName}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <Building2 size={13} style={{ color: '#3a5240' }} />
      ...
      {(summary.pivotsWithAlerts > 0 || summary.pivotsWithClimateFallback > 0) && (
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 260 }}>
            <p style={{ fontSize: 13, fontWeight: 600, color: '#ecefec' }}>Resumo operacional do parque</p>
      ...
      {Object.keys(lastManagementBySeason).length === 0 && activePivots > 0 && (
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 13, fontWeight: 600, color: '#ecefec' }}>Aguardando primeiro registro</p>
            <p style={{ fontSize: 12, color: '#535c3e', marginTop: 2 }}>
              O cron automático registra o balanço às 02h BRT. Você também pode registrar manualmente em{' '}
              <strong style={{ color: '#7a9e82' }}>Manejo Diário</strong>.
            </p>
          </div>
          <Link href="/manejo" style={{
            display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px',
